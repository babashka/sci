...
#?(:cljr nil :default (def cljs-resolve (resolve 'cljs.analyzer.api/resolve)))

#?(;; for self-hosted
   :cljs (def elide-vars false)
   :default (def elide-vars (= "true" (#?(:clj System/getenv :cljr System.Environment/GetEnvironmentVariable)
                                       "SCI_ELIDE_VARS"))))
...
(macros/deftime
  ...
  (defn var-meta [&env sym opts & _a]
    (let [sym (dequote sym)
          ...
          varm (merge (cond-> {:name (or nm (list 'quote (symbol (name sym))))}
                      ...
                      (let [#?@(:cljs []
                                :default [the-var (macros/? :clj (resolve fqsym)
                                                            :cljs (atom nil))])]
                        (macros/? :clj #?(:cljs nil
                                          :default (let [m (meta the-var)
                                                         dyn (:dynamic m)
                                                         arglists (:arglists m)]
  ...
  (defmacro copy-var
    ...
    (let [macro (:macro opts)
          #?@(:cljs []
              :default [the-var (macros/? :clj (resolve sym)
                                          :cljs (atom nil))])
          dyn (:dynamic opts)
      ...
      (if macro
        (macros/? :clj
                  #?(:cljs `(sci.lang.Var. ~init ~nm ~varm false ~ctx nil)
                     :default  `(sci.lang.Var. ~(deref the-var) ~nm ~varm false ~ctx nil))
                  :cljs `(sci.lang.Var. ~init ~nm ~varm false ~ctx nil))
        (if elide-vars
            (if (or dyn ctx)
