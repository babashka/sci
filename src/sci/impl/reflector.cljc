...
#?(:cljs nil :default (set! *warn-on-reflection* :warn-on-boxed))
...
#?(:clj
   ...
   (defn get-methods
     [& args]
     (throw (ex-info (str "TODO " `get-methods) {}))))
...
#?(:clj
   ...
   (defn get-static-field
     [& args]
     (throw (ex-info (str "TODO " `get-static-field) {}))))
...
#?(:clj
   ...
   (defn invoke-constructor
     [& args]
     (throw (ex-info (str "TODO " `invoke-constructor) {}))))
...
#?(:clj
   ...
   (defn invoke-static-method
     [& args]
     (throw (ex-info (str "TODO " `invoke-static-method) {}))))
...
#?(:clj
   (defn- match-method
     ...
     [methods args arg-types]
     (loop [methods methods
            found-m nil]
       (if-let [^Method m (first methods)]
         (let [params (.getParameterTypes m)]
           (if (and (is-congruent? params args arg-types)
             ...
             (recur (rest methods) m)
             (recur (rest methods) found-m)))
         found-m))))
...
#?(:clj
   (defn invoke-matching-method
     ...
     ([method-name methods context-class target args arg-types]
      (let [methods (seq methods)]
        (if (empty? methods)
          (throw (IllegalArgumentException.
   ...
   (defn invoke-matching-method
     [& args]
     (throw (ex-info (str "TODO " `invoke-matching-method) {}))))
